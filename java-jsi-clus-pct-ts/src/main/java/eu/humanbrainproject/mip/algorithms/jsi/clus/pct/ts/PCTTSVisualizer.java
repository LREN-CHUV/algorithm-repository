 ...
 * 
 * This class produces javascript for PCT-TS visualization
 */
public class PCTTSVisualizer extends ClusVisualizationSerializer<ClusNode> {
	...
	private void visualizationRecursive(ClusNode model, StringBuilder nodes, StringBuilder edges) {
		...
		if (model.atBottomLevel()) {
			// add leaf node
			nodes.append(String.format("nodes.push({id: %d, label: '%s'});", model.getID(),
					model.getTargetStat().getPredictString()));
		} else {
			// add intermediate node
			nodes.append(String.format("nodes.push({id: %d, label: '%s'});", model.getID(), test.getTestString()));
		}
		...
		for (int i = 0; i < model.getNbChildren(); i++) {
			...
			edges.append(
					String.format("edges.push({from: %d, to: %d, label: '%s'});", model.getID(), child.getID(), lbl));
	...
	private String getVisJSCode() {
		return "var container = document.getElementById('visualization');" + "var data = {" + "  nodes: nodes,"
				+ "  edges: edges" + "};" + "var options = {" + "  layout: {" + "  hierarchical: {"
				+ "     sortMethod: layoutMethod" + "   }" + " }," + " edges: {" + "   smooth: true,"
				+ "   arrows: {to : true}" + " }" + "};" + "network = new vis.Network(container, data, options);";
	}
