...
@mock.patch('linear_regression._store_multiple_results')
def test_main(mock_results, mock_fetch_data):
    mock_fetch_data.return_value = t.inputs_regression(include_nominal=True)
    main()

    output = mock_results.call_args[0][0]
    pfa = mock_results.call_args[0][1]
...
@mock.patch('linear_regression.io_helper.fetch_data')
@mock.patch('linear_regression._store_multiple_results')
def test_main_logistic(mock_results, mock_fetch_data):
...
@mock.patch('linear_regression.io_helper.results_complete')
@mock.patch('sys.exit')
def test_main_logistic_single_category(mock_exit, mock_results_complete, mock_save_error, mock_fetch_data):
...
@mock.patch('linear_regression.io_helper.results_complete')
def test_main_empty(mock_results_complete, mock_save_results, mock_fetch_data):
    mock_fetch_data.return_value = t.inputs_regression(include_nominal=True, limit_to=0)
    main()
    output = json.loads(mock_save_results.call_args_list[0][0][0])
...
@mock.patch('linear_regression.io_helper.save_results')
@mock.patch('linear_regression.io_helper.results_complete')
def test_aggregate(mock_results_complete, mock_save_results, mock_get_results, mock_fetch_data):
    # run partial jobs
    ...
    # run computations
    aggregate(['1', '2'])
    output_agg = json.loads(mock_save_results.call_args_list[-2][0][0])
    ...
    main()
    output_single = json.loads(mock_save_results.call_args_list[-2][0][0])
...
@mock.patch('linear_regression.io_helper.save_results')
@mock.patch('linear_regression.io_helper.results_complete')
def test_aggregate_single(mock_results_complete, mock_save_results, mock_get_results, mock_fetch_data):
